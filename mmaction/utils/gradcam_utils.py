...
class GradCAM:
    ...
    def _calculate_localization_map(self, inputs, use_labels, delta=1e-20):
        ...
        #localization_map = torch.sum(
        #    activations, dim=2, keepdim=True)
    ...
    def _alpha_blending(self, localization_map, input_imgs, alpha):
        ...
        #print('is_recognizer2d:', self.is_recognizer2d)
        if self.is_recognizer2d:
        ...
        #shift_r = curr_inp.view(-1,t,h//2,2,w//2,2,c)
        #print('curr',curr_inp.shape)
        #curr_inp[:,:,0::2,0::2,:] = torch.roll(curr_inp[:,:,0::2,0::2,:],shifts=-1*6,dims=1)
        #curr_inp[:,:,1::2,1::2,:] = torch.roll(curr_inp[:,:,1::2,1::2,:],shifts=1*6,dims=1)
        #curr_inp[:, :, 0::3, 1::3,:] = torch.roll(curr_inp[:, :,0::3,1::3,:], shifts=1, dims=1)  # top left
        #curr_inp[:, :, 1::3, 0::3,:] = torch.roll(curr_inp[:, :,1::3,0::3,:], shifts=-1, dims=1)  # top left
        #curr_inp[:, :, 0::3, 2::3,:] = torch.roll(curr_inp[:, :,0::3,2::3,:], shifts=2, dims=1)  # top left
        #curr_inp[:, :, 2::3, 0::3,:] = torch.roll(curr_inp[:, :,2::3,0::3,:], shifts=-2, dims=1)  # top left
        #curr_inp[:, :, 1::3, 2::3,:] = torch.roll(curr_inp[:, :,1::3,2::3,:], shifts=3, dims=1)  # top left
        #curr_inp[:, :, 2::3, 1::3,:] = torch.roll(curr_inp[:, :,2::3,1::3,:], shifts=-3, dims=1)  # top left
        #curr_inp[:, :, 2::3, 2::3,:] = torch.roll(curr_inp[:, :,2::3,2::3,:], shifts=4, dims=1)  # top left
        # alpha blending
        #blended_imgs = alpha * heatmap + (1 - alpha) * curr_inp
        blended_imgs = curr_inp
