...
class WindowAttention3D(nn.Module):
    ...
    def forward(self, x, mask=None, batch_size=8, frame_len=8):
        ...
        relative_position_bias = relative_position_bias.permute(0, 3, 1, 2).contiguous()  # Frames, nH, Wd*Wh*Ww, Wd*Wh*Ww

        attn = attn.view(batch_size,frame_len,-1,self.num_heads,N,N).permute(0,2,1,3,4,5) + relative_position_bias.unsqueeze(0).unsqueeze(1) # B_, nH, N, N
...
class PatchShift(nn.Module):
    def __init__(self, n_div=8, inv=False, ratio=1):
        ...
        self.ratio = ratio
        if inv:
            print('=> Using inverse PatchShift, head_num: {}, ratio {}, tps'.format(self.fold_div, ratio))
        else:
            print('=> Using bayershift, head_num: {}, ratio {}, tps'.format(self.fold_div, ratio))

    def forward(self, x, batch_size, frame_len):
        x = self.shift(x, fold_div=self.fold_div, inv=self.inv, ratio=self.ratio, batch_size=batch_size,frame_len=frame_len)
        return x #self.net(x)
...
class SwinTransformerBlock3D(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(2,7,7), shift_size=(0,0,0),
        ...
        self.attn = WindowAttention3D(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, shift=self.shift, shift_type=self.shift_type)
    ...
    def forward_part1(self, x, mask_matrix):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, mask=attn_mask, batch_size=B, frame_len=D)  # B*nW, Wd*Wh*Ww, C
        # merge windows
